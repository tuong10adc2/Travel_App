...
import { computeGeoItinerary, type PlaceScheduleInfo, type DayPlan } from "@/lib/itinerary-planner";
...
// Gọi cross-origin từ bản Flutter Web (deploy riêng, domain khác `web-nu-woad-82.vercel.app`) —
// app Flutter Android/iOS gọi HTTP thuần không bị CORS nên không cần header này, chỉ trình
// duyệt (webapp tự nó cùng origin, và Flutter Web) mới cần. An ninh thật nằm ở verify Firebase
// ID token bên dưới, không phải ở CORS, nên cho phép mọi origin gọi route này.
const CORS_HEADERS = {
...
- Khi người dùng muốn LẬP LỊCH TRÌNH cụ thể nhiều ngày (vd "lên lịch 3 ngày ở Sa Pa"), PHẢI gọi tool plan_itinerary thay vì suggest_places — chỉ cần chọn đúng địa điểm phù hợp, KHÔNG tự sắp xếp thứ tự hay đoán khoảng cách/giờ giấc, server sẽ tính bằng toạ độ thật + giờ mở cửa rồi trả lại cho bạn để bạn diễn giải bằng lời. Kết quả mỗi ngày kèm giờ đến/đi dự kiến từng điểm ("schedule") và "warnings" nếu có điểm có thể bị ghé lúc đã đóng cửa — nếu có warnings, PHẢI nhắc người dùng trong câu trả lời (ngắn gọn), không được im lặng bỏ qua.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof UnauthorizedError) {
      return jsonWithCors({ error: "Bạn cần đăng nhập để trò chuyện với trợ lý du lịch." }, { status: 401 });
    }
    throw error;
  }
  ...
  if (!message) {
    return jsonWithCors({ error: "Thiếu nội dung tin nhắn." }, { status: 400 });
  }
  if (message.length > MAX_MESSAGE_LENGTH) {
    return jsonWithCors({ error: `Tin nhắn quá dài (tối đa ${MAX_MESSAGE_LENGTH} ký tự).` }, { status: 400 });
  }
  ...
  try {
    ...
    const placesInfoById = new Map<string, PlaceScheduleInfo>();
    ...
    const placesList = placesSnap.docs
      .map((doc) => {
        ...
        if (location && typeof location.latitude === "number" && typeof location.longitude === "number") {
          placesInfoById.set(doc.id, {
            id: doc.id,
            lat: location.latitude,
            lng: location.longitude,
            visitDurationMinutes: typeof d.visitDurationMinutes === "number" ? d.visitDurationMinutes : undefined,
            openingHours: d.openingHours && typeof d.openingHours === "object" ? d.openingHours : undefined,
          });
        }
    ...
    // Stream SSE thay vì đợi trả lời đầy đủ rồi mới gửi 1 lần — trải nghiệm giống ChatGPT/Claude.ai
    // (chữ hiện dần), thay vì đợi cả câu như trước. Vòng lặp tool-use giữ nguyên logic cũ, chỉ khác
    // là mỗi đoạn text sinh ra được đẩy ngay ra client qua `send()` thay vì gộp rồi trả 1 lần cuối.
        ...
        try {
          ...
          for (let turn = 0; turn < MAX_TOOL_ROUND_TRIPS; turn++) {
            ...
            for (const block of response.content) {
              if (block.type === "tool_use" && block.name === "suggest_places") {
                ...
                itineraryPlan = computeGeoItinerary(ids, days, placesInfoById);
    ...
    console.error("api/chat: loi khi chuan bi du lieu", error);
    return jsonWithCors({ error: "Đã có lỗi khi gọi trợ lý AI. Vui lòng thử lại." }, { status: 500 });
  }
}
